...
import { EventStatus, InvitationStatus, EventType, PrismaClient } from '@prisma/client';
...
async function main() {
    ...
    const hashedPassword = await bcrypt.hash(faker.internet.password({ length: 20, memorable: true }), 10);
    ...
    for (let index = 0; index < 24; ++index) {
      ...
      users.push(await prisma.user.create({
        data: {
          ...
          email: fakeFirstName.toLowerCase()
            + '.' + fakeLastName.toLowerCase()
